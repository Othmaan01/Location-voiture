...
export function SearchShell({ filters, results, total, pageCount, cities }: SearchShellProps) {
  ...
  function update(patch: Partial<SearchFilters>, resetPage = true) {
    const next: SearchFilters = { ...filters, ...patch, page: resetPage ? 1 : (patch.page ?? filters.page) };
    startTransition(() => {
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Input
            ...
            onBlur={(e) => {
              if ((e.target.value || undefined) !== filters.q) update({ q: e.target.value || undefined });
            }}
          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <div className="inline-flex rounded-full border border-ink-200 p-0.5 xl:hidden">
              {(["liste", "carte"] as const).map((view) => (
                ...
                >
                  {view === "liste" ? <List className="size-3.5" /> : <MapIcon className="size-3.5" />}
                  {view}
                </button>
              ))}
            </div>
          ...
          <div className="grid gap-5 xl:grid-cols-[1fr_26rem]">
            ...
            >
              <MapCanvas
                ...
                onSelect={(id) => {
                  setActiveId(id);
                  document.getElementById(`vehicule-${id}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
                }}
              />
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute inset-x-0 bottom-0 flex max-h-[86dvh] flex-col rounded-t-2xl bg-surface shadow-lift">
            ...
            <div className="flex items-center justify-between border-b border-ink-100 p-4">
              ...
              <Button variant="ghost" size="icon" onClick={() => setFiltersOpen(false)} aria-label="Fermer">
                <X />
              </Button>
            </div>
