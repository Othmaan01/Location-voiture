...
- **Site public** : https://karson.fr (Netlify, site `karson-859`, domaine IONOS ; `apps/web/netlify.toml`, deploiement `netlify deploy --build --prod` depuis `apps/web`). Fly (`location-voiture-web-staging`) reste en secours. Depuis le Mac : `~/.fly/bin/flyctl deploy --config apps/web/fly.toml --dockerfile apps/web/Dockerfile --remote-only .` (variables publiques dans `apps/web/fly.toml`).
