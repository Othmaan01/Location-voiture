...
## Décision
Monorepo pnpm + Turborepo : `apps/mobile` (Expo SDK 56, expo-router), `apps/web` (Next.js 16, code RentMap déplacé), `apps/api` (Fastify 5, Drizzle, Zod, pg-boss), `packages/{contracts,pricing,db,tokens,config}`. Une seule app Expo pour client et pro, segments de routes séparés.
