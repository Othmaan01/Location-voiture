  ...
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={jsonLd(vehicleSchema(vehicle, agency))}
      />
      <script
        type="application/ld+json"
      ...
      <div className="container-page py-8 pb-28 lg:pb-8">
        <nav className="mb-5 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/recherche" className="hover:text-ink-900">
            Recherche
          </Link>
          <span>/</span>
          <Link href={`/agence/${agency.slug}`} className="hover:text-ink-900">
            {agency.name}
          </Link>
          <span>/</span>
              ...
              <ul className="space-y-2 text-sm text-ink-600">
                ...
                {vehicle.min_license_years ? (
                  <li className="flex items-center gap-2">
                    ...
                    <strong className="font-medium text-ink-900">
                      {vehicle.min_license_years} ans
                    </strong>
                  </li>
                ) : null}
              </ul>
              <p className="mt-4 rounded-xl bg-surface-muted p-3.5 text-xs text-muted-foreground">
                Le contrat de location est conclu directement entre vous et l&apos;agence. RentMap
                ne percoit aucune commission et n&apos;intervient pas dans la transaction.
              </p>
            </section>
          </div>
        ...
        {similar.length > 0 ? (
          <section className="mt-16">
            <h2 className="mb-5 text-xl font-semibold text-ink-900">Vehicules similaires</h2>
